...
def export_data():
    '''
    create a json file with keys  packet : [ Sensor, Medium, Constant ]
    Each Sensor, Medium, Constant keys have values of their index ranging from 0 to the number of that type of packet
    For each index of medium sweeps, there are keys:
    ...
    For each index of constant sweeps there are keys:
        Start - start time of this packet after launch in ms
        Stop - stop time of this packet after launch in ms
        Count - the index of this packet among all packets
    '''
    #call Packet_test to generate the global variable UDIP_packets, as well as the global variables ind_sensor, ind_medium,
    #ind_large, ind_burst, ind_constant. We care about ind_sensor, ind_medium, and ind_constant. These contain the indicies 
    #within UDIP_packets where that type of packet resides
    Packet_test()
    print(f"Len of Sensor: {len(ind_sensor)}\nLen of Medium: {len(ind_medium)}\nLen of Large: {len(ind_large)}\n Len of Burst: {len(ind_burst)}\nLen of Constant: {len(ind_constant)}")
    json_obj = { "packets" : {"Sensor" : {}, "Medium" : {}, "Constant" : {}} }

    print("Adding sensor packets")
    for i in range(len(ind_sensor)):
        pckt = UDIP_packets[ind_sensor[i]]
        json_obj["packets"]["Sensor"][i] = {"Start" : pckt.tInitial, "Stop" : pckt.tFinal, "Count" : pckt.count,
                                            "Acceleration" : [pckt.accX, pckt.accY, pckt.accZ, pckt.accH], 
                                            "Spin Rate" : [pckt.gyroX, pckt.gyroY, pckt.gyroZ],
                                            "Magnetic Field" : [pckt.magX, pckt.magY, pckt.magZ],
                                            "Temperature" : [pckt.temperature_p, pckt.temperature_s, pckt.temperature_d],
                                            "Photo Diode" : [pckt.pd_1, pckt.pd_2]}    
    print("Adding medium sweep packets")
    for i in range(len(ind_medium)):
        pckt = UDIP_packets[ind_medium[i]]
        json_obj["packets"]["Medium"][i] = {"Start" : pckt.tInitial, "Stop" : pckt.tFinal, "Count" : pckt.count,
                                            "Voltage" : pckt.sweep.sweepVoltage.tolist(), 'Current0' : pckt.sweep.adc0Curr.tolist(),
                                            "Current1" : pckt.sweep.adc1Curr.tolist(), "Current2" : pckt.sweep.adc2Curr.tolist()}
    print("Adding constant sweep packets")
    for i in range(len(ind_constant)):
        pckt = UDIP_packets[ind_constant[i]]
        json_obj["packets"]["Constant"][i] = {"Start" : pckt.tInitial, "Stop" : pckt.tInitial, "Count" : pckt.count,
                                              "Voltage" : pckt.sweep.ProbeVolt.tolist(), "Photo Diode 1" : pckt.sweep.adcPD1Volt.tolist(),
                                              "Photo Diode 2" : pckt.sweep.adcPD2Volt.tolist(), "Current0" : pckt.sweep.adc0Curr.tolist(), 
                                              "Current1" : pckt.sweep.adc1Curr.tolist(), "Current2" : pckt.sweep.adc2Curr.tolist()}
    json_fileName = "2022_packet.json"

...
import UDIP_Lib.UDIP_4_Lib_V2 as UDIP_Lib  # making updating UDIP_Lib easier

# from datetime import datetime

# 1. Initialization

filename = "UDIP2024/UDIP123.dat"
Outpath = "Analysis/"
...
def Plot_Percentiles(start=None, stop=None, percent=95):
    Packet_test()  # load data only if needed
    # need to be updated!!!!!!!

    S_type = UDIP_Lib.typeFull_Probe
    ...
    #     typ = 'Burst'
    if S_type == UDIP_Lib.typeFull_Probe:
        typ = 'FullProbe'
    elif S_type == UDIP_Lib.typeFull_Rckt:
        typ = 'FullRocket'
    elif S_type == UDIP_Lib.typeDens_Probe:
        typ = 'DenseProbe'
    elif S_type == UDIP_Lib.typeDens_Rckt:
        typ = 'DenseRocket'
...
def Plot_Sensors(start, stop, accel=False, accelH=False, gyro=False, magno=False, temp=False, photo=False):
    ...
    startSec = round(start / 1000)
    stopSec = round(stop / 1000)
    ...
    beginning = find_closest(start, ind_sensor)
    end = find_closest(stop, ind_sensor)

    for i in ind_sensor:
        if (i >= ind_sensor[beginning]) and (i <= ind_sensor[end]):
    ...
    for i in target:
        ...
        AccHiRng.append(UDIP_packets[i].accHiRng)
        ...
        Temperature.append(UDIP_packets[i].temperature)
        # Pd_1.append(UDIP_packets[i].pd_1)
        # Pd_2.append(UDIP_packets[i].pd_2)
        PhDiode.append(UDIP_packets[i].phDiode)
    ...
    if (accel):
        ...
        if (accelH):
            plt.plot(np.divide(time_started, 1000), AccHiRng, label="H")
        plt.title("Accelerometer")
        # plt.plot(np.divide(time_started,1000),AccZ_ana,label="Z Acceleration analog")
        plt.ylabel("Acceleration (m/s^2)")
        plt.xlabel("Time (sec)")
        plt.legend(loc="lower right")
        plt.savefig(Outpath + 'acceleration_from_{}sec_to_{}sec.png'.format(startSec, stopSec))
        plt.close()
    ...
    if (gyro):
        ...
        plt.title('Gyroscope')
        plt.ylabel("Spin rate (degrees/millisec)")
        plt.xlabel("Time (sec)")
        plt.legend(loc="upper left")
        plt.savefig(Outpath + 'gyro_from_{}sec_to_{}sec.png'.format(startSec, stopSec))
        plt.close()
    ...
    if (magno):
        ...
        plt.xlabel("Time (sec)")
        plt.legend(loc="upper left")
        plt.savefig(Outpath + 'mag_from_{}sec_to_{}sec.png'.format(startSec, stopSec))
        plt.close()
    ...
    if (temp):
        ...
        plt.plot(np.divide(time_started, 1000), Temperature)
        plt.title('Temperature')
        plt.ylabel("Temperature (C)")
        plt.xlabel("Time (sec)")
        plt.legend(loc="upper left")
        plt.savefig(Outpath + 'temperature_from_{}sec_to_{}sec.png'.format(startSec, stopSec))
        plt.close()

    if (photo):
        # plt.plot(np.divide(time_started,1000),Pd_1,label="Pd 1")
        # plt.plot(np.divide(time_started,1000),Pd_2,label="Pd 2")
        plt.plot(np.divide(time_started, 1000), PhDiode)
        plt.title('Photodiode')
        plt.ylabel("Magnitude (V)")
        plt.xlabel("Time (sec)")
        plt.legend(loc="upper left")
        plt.savefig(Outpath + 'photo_diodes_from_{}sec_to_{}sec.png'.format(startSec, stopSec))
        plt.close()
...
#     # return ind_sensor, ind_full, ind_dense
...
# Plot_sweep(index=800)
# Plot_Many_Sweeps()
# 
# Plot_Percentiles(0,300)
# 
# Plot_Sensors(0, 300000, accel=True, accelH=False, gyro=True, magno=True, temp=True, photo=True)
# Plot_Sensors(0, 50000, accel=True, accelH=False, gyro=True, magno=True)
